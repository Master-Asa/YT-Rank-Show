...
try{
 ...
 if(!$Headless){Add-Type -AssemblyName System.Windows.Forms;[Windows.Forms.Application]::EnableVisualStyles();$taskForm=New-Object Windows.Forms.Form;$taskForm.Text='YT Rank Show 更新';$taskForm.Width=500;$taskForm.Height=190;$taskForm.StartPosition='CenterScreen';$taskLabel=New-Object Windows.Forms.Label;$taskLabel.Dock='Fill';$taskLabel.Padding=New-Object Windows.Forms.Padding(20);$taskLabel.Text='更新包已驗證。請自行關閉 OneComme，關閉後會開始備份與安裝。此時可關閉此視窗取消。';$taskForm.Controls.Add($taskLabel);Add-Type 'using System;using System.Runtime.InteropServices;public static class RankUpdateWindow{[DllImport("user32.dll")]public static extern bool ShowWindow(IntPtr h,int n);[DllImport("user32.dll")]public static extern bool IsWindowVisible(IntPtr h);}';$taskForm.Show();[void][RankUpdateWindow]::ShowWindow($taskForm.Handle,5);[Windows.Forms.Application]::DoEvents();if(![RankUpdateWindow]::IsWindowVisible($taskForm.Handle)){throw 'Update window is not visible'};$taskForm.Activate();WriteStatus 'waiting' '更新視窗已顯示，等待 OneComme 關閉';$ackDeadline=(Get-Date).AddSeconds(90);while(!(Test-Path -LiteralPath (Join-Path $taskDir 'launch-confirmed'))){[Windows.Forms.Application]::DoEvents();if($taskForm.IsDisposed -or (Get-Date) -gt $ackDeadline){WriteStatus 'cancelled' '啟動確認取消或逾時，沒有安裝';exit 0};Start-Sleep -Milliseconds 100};$deadline=(Get-Date).AddMinutes(30);while(!(Stopped)){[Windows.Forms.Application]::DoEvents();if($taskForm.IsDisposed -or (Get-Date) -gt $deadline){WriteStatus 'cancelled' '已取消或等待逾時，沒有安裝';exit 0};Start-Sleep -Milliseconds 500};WriteStatus 'installing' '正在備份與安裝';$taskForm.ControlBox=$false;$taskLabel.Text='正在備份與安裝，請勿重新開啟 OneComme…';[Windows.Forms.Application]::DoEvents()}
